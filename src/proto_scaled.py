...
GENOME="/tmp/residual.fa"; JFDB="/tmp/res16.jf"
...
K=16; SEED_MINCOUNT=200; C_OCC=120; CAP_INST=60; CORE=300
...
def main():
    ...
    def s2c(s):
        ...
        return np.uint32(c)
    seeds={s2c(r[0]):int(r[1]) for r in rows}
    ...
    cf=f"{OUT}/scaled_core.fa"
    with open(cf,"w") as f:
        for i,s in enumerate(core): f.write(f">{i}\n{s}\n")
    subprocess.run([CDHIT,"-i",cf,"-o",f"{OUT}/scaled_clust","-c","0.80","-n","5",
                    "-aS","0.6","-r","1","-d","0","-T","64","-M","48000","-g","1"],
                   stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    clusters=[];cur=None
    for ln in open(f"{OUT}/scaled_clust.clstr"):
        if ln[0]=='>':
    ...
    def extract(members,w):
        ...
        return out
    outc=open(f"{OUT}/scaled_consensi.fa","w"); nf=0
    for c in fams:
        mem=c[:]
        if len(mem)>POA_MAX:
        ...
        nf+=1
        outc.write(f">scaled_fam_{nf} members={len(c)} win={w} len={len(cons)}\n")
