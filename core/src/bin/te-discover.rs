...
fn genomic_tandem(spans: &mut [(u32, i64, i64, bool)], cons_len: i64) -> bool {
    ...
    for i in 1..spans.len() {
        if spans[i].0 == spans[i - 1].0 && spans[i].1 - spans[i - 1].2 < cons_len {
            adj += 1;
        }
    }
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!(
            "usage: te-discover <genome.fa> [k=16] [min_count=200] [cap=200] [out_prefix=te_disc]"
        );
        std::process::exit(1);
    }
    let fasta = &args[1];
    ...
    // greedy: highest count first. Tie-break by seed code so the order — and thus the
    // greedy family partition / masking — is fully deterministic (G10 reproducibility);
    // sort_unstable is not order-preserving, so equal-count seeds need an explicit key.
    seeds.sort_unstable_by(|a, b| b.count.cmp(&a.count).then(a.code.cmp(&b.code)));
    eprintln!(
        "[seed] {} seeds count>={}, {:.1}s",
        seeds.len(),
        min_count,
    ...
    for s in &seeds {
        ...
        let live: Vec<(u32, u32, bool)> = s
            .occ
            .iter()
            .cloned()
            .filter(|&(ri, off, _)| !masked[ri as usize][off as usize])
            .collect();
        if live.len() < MIN_MEMBERS {
            continue;
        }
        ...
        if active.iter().filter(|&&a| a).count() < MIN_MEMBERS {
            continue;
        }
        ...
        cons.extend(&left);
        for i in (0..k).rev() {
            cons.push(((s.code >> (2 * i)) & 3) as u8);
        }
        cons.extend(&right);
        if cons.len() < MIN_CONS_LEN {
            continue;
        }
        ...
        for &(ri, off, st) in &live {
            if !member_match(&recs[ri as usize], off, st, &cons, ll, k) {
                continue;
            }
            let (gs, ge) = if st {
                (off as i64 - ll, off as i64 + k as i64 + rl)
            } else {
                (off as i64 - rl, off as i64 + k as i64 + ll)
            };
        ...
        for &(ri, gs, ge, st) in &spans {
            for x in gs..ge {
                masked[ri as usize][x as usize] = true;
            }
            if !tandem {
                writeln!(
                    bed,
    ...
    tfa.flush().unwrap();
    eprintln!(
        "[A1] {} interspersed families + {} tandem; {} member alignments (O(occ)); {:.1}s",
        nfam,
        n_tandem,
        total_align,
        t0.elapsed().as_secs_f64()
