...
const SPOA: &str = "/home/shuoc/tool/miniconda3/envs/PGTA/bin/spoa";
const MAX_MEMBERS: usize = 30;
const MIN_MEMBERS: usize = 3;

#[inline]
fn code_to_base(c: u8) -> u8 { if c < 4 { b"ACGT"[c as usize] } else { b'N' } }

fn revcomp(s: &[u8]) -> Vec<u8> {
    s.iter().rev().map(|&b| match b { b'A' => b'T', b'C' => b'G', b'G' => b'C', b'T' => b'A', _ => b'N' }).collect()
}
...
fn main() {
    ...
    let idx: HashMap<&str, usize> = recs.iter().enumerate().map(|(i, r)| (r.name.as_str(), i)).collect();
    ...
    for line in BufReader::new(File::open(&args[2]).unwrap()).lines() {
        ...
        if f.len() < 6 { continue; }
        let ri = match idx.get(f[0]) { Some(&i) => i, None => continue };
        let (s, e) = (f[1].parse::<usize>().unwrap(), f[2].parse::<usize>().unwrap());
        let fam = f[3].to_string();
        let plus = f[5] == "+";
        fams.entry(fam.clone()).or_insert_with(|| { order.push(fam.clone()); Vec::new() }).push((ri, s, e, plus));
    }
    ...
    for fam in &order {
        ...
        if mem.len() < MIN_MEMBERS { continue; }
        if mem.len() > MAX_MEMBERS { mem.truncate(MAX_MEMBERS); } // bounded per-family POA
        // write oriented member sequences for spoa
        {
            ...
            for (i, &(ri, s, e, plus)) in mem.iter().enumerate() {
                let seq: Vec<u8> = recs[ri].codes[s..e.min(recs[ri].codes.len())].iter().map(|&c| code_to_base(c)).collect();
        ...
        let o = Command::new(SPOA).args(["-r", "0", &tmp]).output().unwrap();
        let cons: String = String::from_utf8_lossy(&o.stdout).lines()
            .filter(|l| !l.starts_with('>')).collect();
        if cons.len() < 80 { continue; }
        nf += 1;
        writeln!(out, ">{fam}_ref members={} len={}", mem.len(), cons.len()).unwrap();
        for chunk in cons.as_bytes().chunks(80) { out.write_all(chunk).unwrap(); out.write_all(b"\n").unwrap(); }
    }
    out.flush().unwrap();
    let _ = std::fs::remove_file(&tmp);
    eprintln!("[refine] {} families refined via spoa; {:.1}s", nf, t0.elapsed().as_secs_f64());
    println!("refined\t{nf}");
}
